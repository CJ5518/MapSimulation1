Shader "Custom/OceanMod"
{
	...
	SubShader
	{
		Pass
		{
			...
			float _SpecularSmoothness,_Specular;
			...
			float4 frag (v2f i) : SV_Target {
				...
				float specularStrength = lerp(0, _Specular, saturate(shadows * 5));
				...
				// oceanCol += ripple * 0.3;
				// Apply shading and specuar highlight
				...
				oceanCol = lerp(oceanCol, foam.rgb, foam.a * _FoamColour.a);
				// Apply the shadows to the ocean colour
				oceanCol *= lerp(1, shadows, _ShadowStrength);
				// Add ambient colour
				oceanCol = saturate(oceanCol + _Ambient * 0.2);
				// Add rim light
